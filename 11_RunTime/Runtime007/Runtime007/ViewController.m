...
- (void) classTest {
    ...
    /// 创建一类对
    Class WTCat = objc_allocateClassPair([NSObject class], "WTCat", 0);
    
    /// 添加实例变量
    // const char* types= "v@:"
    NSString* name = @"name";
   
    class_addIvar(WTCat, name.UTF8String, sizeof(id), log2(sizeof(id)), @encode(id));
    
    // 添加方法
    class_addMethod(WTCat, @selector(hunting), (IMP)hunting, "v@:");
    
    /// 注册类
    objc_registerClassPair(WTCat);
    
  
    
    // 创建实例对象
    id cat = [[WTCat alloc] init];
